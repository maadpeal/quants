# 🎯 Objetivo Principal:

## 📚 Master en Finanzas Cuantitativas

---

## 🌟 **Módulo 0**: Fundamentos (Estimado: 1-2 meses)

- 🖥️ [**Fundamentos Computacionales**](./Módulo_0_-_Fundamentos/Fundamentos_Computacionales/README.md)
  - **Importancia**: Proporciona una base sólida para el análisis y modelado financiero.
  - **Aplicación**: Automatización de tareas, análisis de datos y desarrollo de algoritmos financieros.
  
- 💰 [**Fundamentos Financieros**](./Módulo_0_-_Fundamentos/Fundamentos_Financieros/README.md)
  - **Importancia**: Esencial para entender cómo funcionan los mercados y las decisiones financieras.
  - **Aplicación**: Valoración de activos, análisis de inversiones y toma de decisiones financieras.
  
- 📈 [**Matemática Básica**](./Módulo_0_-_Fundamentos/Matemática_basica/README.md)
  - **Importancia**: Conocimiento de las herramientas disponibles en el mercado.
  - **Aplicación**: Inversiones, coberturas y estrategias de trading.

---

## 🌟 **Módulo I**: Introducción y Fundamentos Avanzados (Estimado: 3-4 meses)

- 📊 [**Introducción a los Mercados Financieros**](./Módulo_I_-_Introducción_y_Fundamentos_Avanzados/Introducción_a_los_Mercados_Financieros/README.md)
  - **Importancia**: Entender el comportamiento y funcionamiento de los mercados.
  - **Aplicación**: Operar en bolsa, análisis de tendencias y predicción de movimientos del mercado.
  
- 📈 [**Instrumentos Financieros**](./Módulo_I_-_Introducción_y_Fundamentos_Avanzados/Instrumentos_Financieros/README.md)
  - **Importancia**: Conocimiento de las herramientas disponibles en el mercado.
  - **Aplicación**: Inversiones, coberturas y estrategias de trading.

- 📈 [**Fundamentos Matemáticos**](./Módulo_I_-_Introducción_y_Fundamentos_Avanzados/Fundamentos_Matemáticos/README.md)
  - **Importancia**: Conocimiento de las herramientas disponibles en el mercado.
  - **Aplicación**: Inversiones, coberturas y estrategias de trading.

- 📈 [**Fundamentos Computacionales(Avanzado)**](<./Módulo_I_-_Introducción_y_Fundamentos_Avanzados/Fundamentos_Computacionales_(Avanzado)/README.md>)
  - **Importancia**: Conocimiento de las herramientas disponibles en el mercado.
  - **Aplicación**: Inversiones, coberturas y estrategias de trading.

---

## 🌟 **Módulo II**: Aplicaciones y Especialización (Estimado: 4-5 meses)

- 🖥️ [**CAPITAL ECONÓMICO Y BENCHMARKING**](./Módulo_II_-_Aplicaciones_y_Especialización/Capital_Económico_y_Benchmarking/README.md)
  - **Importancia**: Profundizar en técnicas avanzadas de programación y análisis.
  - **Aplicación**: Desarrollo de software financiero avanzado y análisis de grandes conjuntos de datos.

- 🖥️ [**Fundamentos DE Valoración**](./Módulo_II_-_Aplicaciones_y_Especialización/Fundamentos_de_Valoración/README.md)
  - **Importancia**: Profundizar en técnicas avanzadas de programación y análisis.
  - **Aplicación**: Desarrollo de software financiero avanzado y análisis de grandes conjuntos de datos.

- 🖥️ [**Fundamentos Computacionales**](./Módulo_II_-_Aplicaciones_y_Especialización/Fundamentos_Computacionales/README.md)
  - **Importancia**: Profundizar en técnicas avanzadas de programación y análisis.
  - **Aplicación**: Desarrollo de software financiero avanzado y análisis de grandes conjuntos de datos.

---

## 📌 **Adicionales @--** (Estimado: 1-2 meses):

- 📉 [**Análisis Cuantitativo**](./Adicionales/Análisis_Cuantitativo/README.md)
  - **Importancia**: Evaluar riesgos, retornos y tomar decisiones basadas en datos.
  - **Aplicación**: Modelado financiero, análisis de riesgo y evaluación de inversiones.

- 📉 [**Gestión de proyectos**](./Adicionales/Gestión_de_Proyectos_y_Habilidades_Blandas/README.md)
  - **Importancia**: Evaluar riesgos, retornos y tomar decisiones basadas en datos.
  - **Aplicación**: Modelado financiero, análisis de riesgo y evaluación de inversiones.

- 📉 [**Herramientas financieras**](./Adicionales/Herramientas_Financieras_Específicas/README.md)
  - **Importancia**: Evaluar riesgos, retornos y tomar decisiones basadas en datos.
  - **Aplicación**: Modelado financiero, análisis de riesgo y evaluación de inversiones.
  
# 🎯 Línea de Tiempo: Master en Finanzas Cuantitativas

▶️ Start  
════════════════🖥️ **Módulo 0**: Fundamentos (1-2 meses)  
════════════════════════════════════════📊 **Módulo I**: Introducción y Fundamentos Avanzados (3-4 meses)  
════════════════════════════════════════════════════════🖥️ **Módulo II**: Aplicaciones y Especialización (4-5 meses)  
══════════════📉 **Adicionales @--** (1-2 meses)  
▶️ End

---

## Ponderación por item

| **Módulo/Categoría**                                           | **Ponderación** |
| -------------------------------------------------------------- | --------------- |
| Módulo 0: Fundamentos                                          |                 |
| A. Fundamentos Computacionales                                 |                 |
| Excel                                                          | 7               |
| Python (Básico)                                                | 9               |
| Bases de Datos                                                 | 6               |
| B. Fundamentos Financieros                                     |                 |
| Aritmética Financiera                                          | 8               |
| Módulo I: Introducción y Fundamentos Avanzados                 |                 |
| A. Introducción a los Mercados Financieros                     |                 |
| Mercados de Capitales                                          | 9               |
| Entidades Financieras y de Seguros                             | 7               |
| Contabilidad                                                   | 6               |
| Riesgos Financieros                                            | 8               |
| B. Instrumentos Financieros                                    |                 |
| Mercados e Instrumentos Financieros                            | 9               |
| Futuros, Forwards y Opciones                                   | 8               |
| Índices sobre Tipos de Interés                                 | 7               |
| Interest Rate Swaps y Curvas de Tipos                          | 8               |
| Caps, Floors y Swaptions                                       | 7               |
| Curva de Probabilidades de Solvencia e Instrumentos de Crédito | 6               |
| C. Fundamentos Computacionales (Avanzados)                     |                 |
| Python (Avanzado)                                              | 9               |
| D. Fundamentos Matemáticos                                     |                 |
| Probabilidad y Simulación                                      | 8               |
| Procesos Estocásticos                                          | 7               |
| Módulo II: Aplicaciones y Especialización                      |                 |
| A. Fundamentos Computacionales                                 |                 |
| Cálculo Numérico                                               | 8               |
| Técnicas de Simulación                                         | 9               |
| Cálculo Numérico Avanzado                                      | 8               |
| B. Fundamentos de Valoración                                   |                 |
| Valoración por Replicación y No Arbitraje                      | 9               |
| Modelos Discretos                                              | 7               |
| Entorno Black                                                  | 8               |
| Modelo Normal y Shifted-Lognormal                              | 7               |
| C. Riesgos                                                     |                 |
| Riesgo de Mercado                                              | 9               |
| Riesgo de Crédito y Operacional                                | 8               |
| D. Capital Económico y Benchmarking                            |                 |
| Modelos Regulatorios                                           | 7               |
| E. Adicionales @--                                             |                 |
| Análisis Cuantitativo                                          | 8               |
| Herramientas Financieras Específicas                           | 7               |
| Gestión de Proyectos y Habilidades Blandas                     | 6               |

---

## Listado de contenidos: (completo)

## Módulo 0: Fundamentos

### A. Fundamentos Computacionales: 🟢

1. **Excel** _(Ponderación: 7)_ 🟠
   - Fórmulas básicas.
   - Funciones avanzadas.
   - Tablas pivotantes.
   - Gráficos y análisis de datos.
2. **Python (Básico)** _(Ponderación: 9)_ 🔴
   - Sintaxis fundamental.
   - Variables y tipos de datos.
   - Estructuras de control: loops y condicionales.
   - Funciones y módulos básicos.
3. **Bases de Datos** _(Ponderación: 6)_ 🟢
   - Conceptos básicos de bases de datos.
   - SQL: consultas, inserciones, actualizaciones.
   - Diseño básico de bases de datos.
   - Normalización.

### B. Fundamentos Financieros: 🟢

1. **Aritmética Financiera** _(Ponderación: 8)_ 🟠
   - Interés simple y compuesto.
   - Valor presente y futuro.
   - Tasas de interés y descuento.

---

## Módulo I: Introducción y Fundamentos Avanzados

### A. Introducción a los Mercados Financieros: 🟢

1. **Mercados de Capitales** _(Ponderación: 9)_ 🔴

   - Estructura y funcionamiento.
   - Actores principales y su rol.

2. **Entidades Financieras y de Seguros** _(Ponderación: 7)_ 🟢

   - Tipos de entidades.
   - Funcionamiento y regulación.

3. **Contabilidad** _(Ponderación: 6)_ 🟢

   - Principios básicos de contabilidad.
   - Balances y estados de resultados.

4. **Riesgos Financieros** _(Ponderación: 8)_ 🟠
   - Tipos de riesgos.
   - Gestión y mitigación.

### B. Instrumentos Financieros: 🟢

1. **Mercados e Instrumentos Financieros** _(Ponderación: 9)_ 🔴

   - Tipos de mercados: primarios, secundarios.
   - Instrumentos: acciones, bonos, derivados.

2. **Futuros, Forwards y Opciones** _(Ponderación: 8)_ 🟠
   - Definición y diferencias.
   - Uso y aplicación en finanzas.
3. **Índices sobre Tipos de Interés** _(Ponderación: 7)_ 🟢

   - Concepto y aplicación.
   - Principales índices y su impacto.

4. **Interest Rate Swaps y Curvas de Tipos** _(Ponderación: 8)_ 🟠

   - Definición de swap de tasa de interés.
   - Construcción y uso de curvas de tipos.

5. **Caps, Floors y Swaptions** _(Ponderación: 7)_ 🟢

   - Instrumentos derivados de tasas de interés.
   - Aplicaciones y valoración.

6. **Curva de Probabilidades de Solvencia e Instrumentos de Crédito** _(Ponderación: 6)_ 🟢
   - Concepto de solvencia.
   - Instrumentos para medir y gestionar el crédito.

### C. Fundamentos Computacionales (Avanzados): 🟢

1. **Python (Avanzado)** _(Ponderación: 9)_ 🔴
   - Bibliotecas especializadas: Pandas, NumPy, SciPy.
   - Programación orientada a objetos.
   - Integración con bases de datos y otras herramientas.

### D. Fundamentos Matemáticos: 🟢

1. **Probabilidad y Simulación** _(Ponderación: 8)_ 🟠

   - Variables aleatorias.
   - Modelos multidimensionales.
   - Cópulas.
   - Sumas estocásticas.
   - Teorema central del límite.

2. **Procesos Estocásticos** _(Ponderación: 7)_ 🟢
   - Procesos discretos: camino aleatorio, martingalas, cadenas de Markov.
   - Procesos continuos: movimiento browniano, procesos de Ito y Poisson, lema de Ito.
   - Conexión con EDPs.

---

## Módulo II: Aplicaciones y Especialización

### A. Fundamentos Computacionales: 🟢

1. **Cálculo Numérico** _(Ponderación: 8)_ 🟠

   - Álgebra matricial numérica.
   - Resolución de ecuaciones.
   - Integración y derivación numéricas.
   - Interpolación.

2. **Técnicas de Simulación** _(Ponderación: 9)_ 🔴

   - Reducción de varianza.
   - Muestreo por importancia.
   - Ejercicio anticipado.

3. **Cálculo Numérico Avanzado** _(Ponderación: 8)_ 🟠
   - Resolución de ecuaciones derivadas en parciales.
   - Técnicas de optimización.

### B. Fundamentos de Valoración: 🟢

1. **Valoración por Replicación y No Arbitraje** _(Ponderación: 9)_ 🔴

   - Conceptos básicos y aplicación.

2. **Modelos Discretos** _(Ponderación: 7)_ 🟢

   - Modelos matriciales.
   - Árboles binomiales.

3. **Entorno Black** _(Ponderación: 8)_ 🟠

   - Aplicación para equity, forwards y swaps.
   - Ajustes de convexidad.

4. **Modelo Normal y Shifted-Lognormal** _(Ponderación: 7)_ 🟢
   - Diferencias y aplicaciones.

---

## C. Riesgos: 🟢

1. **Riesgo de Mercado** _(Ponderación: 9)_ 🔴

   - Metodologías de medición.
   - Marco y novedades regulatorias.

2. **Riesgo de Crédito y Operacional** _(Ponderación: 8)_ 🟠
   - Modelado y drivers de riesgo.
   - Marco regulatorio.

---

## D. Capital Económico y Benchmarking: 🟢

1. **Modelos Regulatorios** _(Ponderación: 7)_ 🟢
   - Comparación y análisis de modelos financieros regulatorios.

---

## E. Adicionales @--: 🟢

1. **Análisis Cuantitativo** _(Ponderación: 8)_ 🟠

   - Estadísticas descriptivas.
   - Modelado predictivo.
   - Backtesting y ajuste de modelos.

2. **Herramientas Financieras Específicas** _(Ponderación: 7)_ 🟢

   - Plataformas de trading algorítmico.
   - Herramientas de análisis de riesgo.

3. **Gestión de Proyectos y Habilidades Blandas** _(Ponderación: 6)_ 🟢
   - Comunicación efectiva con stakeholders.
   - Presentación de resultados y hallazgos.
   - Gestión de proyectos en entornos financieros.
